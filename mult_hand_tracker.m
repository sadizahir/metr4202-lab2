...
handDetector = vision.CascadeObjectDetector('pun_detector.xml','MergeThreshold',20); % Finds faces by default
...
while isempty(bboxes)
    ...
    bboxes = handDetector.step(frame);
end
...
while keepRunning
    ...
    if mod(frameNumber, 10) == 0
        ...
        % downsampled frame:
        % bboxes = handDetector.step(frame);
        bboxes = 2 * handDetector.step(imresize(frame, 0.5));
        if ~isempty(bboxes)
            tracker.addDetections(frame, bboxes);
        end
    else
